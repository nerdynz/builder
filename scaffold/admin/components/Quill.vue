<template>
  <div>
    ...
    <div v-show="false && (isDev || globalIsDev)">
      <textarea :value="value" @input="(val) => {this.$emit('input', val)}"></textarea>
    </div>
  </div>
</template>
...
export default {
  props: {
    ...
    placeholder: {
      type: String,
      default: 'Compose something awesome...'
    },
    autofocus: Boolean
  },
  computed: {
    ...mapGetters({
      'globalIsDev': 'auth/userIsDev'
    }),
    options () {
      var toolbar = [
        ['clean'],
...
.quill-editor.ql-container .ql-placeholder {
  padding: 5px !important;
  min-height: 80px;
}

.quill-editor.quill-placeholder {
  background: $white;
