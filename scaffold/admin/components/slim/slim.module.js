...
var detachChildren = function(element) {
  ...
  for(i=0;i<len;++i) {
    el = animations[i][0];
    if (element && element.contains(el) || element === el) {
      elements.push(el);
    }
  }
...
var Slim = function () {
   ...
   */
		...
		value: function _doUpload(callback) {
			...
			console.log(this._data.input.image)
		...
		value: function _doDestroy() {
			...
			if (this._btnGroup && this._btnGroup.children.length > 0) {
				nodeListToArray(this._btnGroup.children).forEach(function (btn) {
   ...
   */
		...
		value: function options() {
			...
			var defaults = {
				...
				label: '<p><span class="icon"><i class="fa fa-cloud-download"></i></span></p><p>Click or Drop<br> Image</p>',
